...
Description
----
The user component manages the user account and their authorizations. In the traditional sense, this could be a use that belongs to certain groups, which in turn have certain rights. The component also supports the maintenance and validation of passwords (in order to facilitate login options on a dashboard or application, for example) but in the somewhat abstract context it also supports general login systems.

For example, the user component itself can function as an oAuth 2.0 source and thus be the source holder for an OpenId of a user, but it can also handle alternative login flows such as AFC and SAML (and thus DigiD). Finally, it can function as a receiving party of oAuth login and thus offers a login with facebook, Google or Github option for applications.
